...
class Model:
    ...
    def forward_pass(self, inputs):
        ...
        loss = inputs.reshape((1,len(inputs)))@self.W + self.b
        # TODO: Get probabilities by using softmax on the logits
        probabilities = np.zeros((1,self.num_classes))
        biggest = np.max(loss)
        subtract = np.full((1,len(loss)), biggest)
        temp = np.power(np.e, loss-subtract)
        probabilities = temp/np.sum(temp)
                
        return np.reshape(probabilities, (1,self.num_classes))
    ...
    def loss(probabilities, gt_label):
        ...
        loss = np.log(probabilities[0][gt_label]+0.0000000001)
        return -1*loss
    ...
    def back_propagation(img, probabilities, gt_label):
        ...
        """
        gradW = np.zeros((len(img), len(probabilities[0])))
        gradB = np.zeros((1, len(probabilities[0])))
        # TODO: Back propagation: use gradient descent to update parameters
        remove = np.zeros((1, len(probabilities[0])))
        remove[0][gt_label] = 1

        # TODO: Reshape train image data to be matrix, dimension [784, 1]
        gradW = img.reshape((len(img), 1))@(probabilities-remove)
        gradB = probabilities - remove
        return gradW, gradB
    ...
    def train_nn(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for index in range(len(indices)):
                ...
                loss_sum = loss_sum + self.loss(probabilities, gt_label)
